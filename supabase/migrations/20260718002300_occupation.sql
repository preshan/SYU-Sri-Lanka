-- Optional occupation / job title on profiles.
...
alter table public.profiles
  add constraint profiles_occupation_len
  check (
    occupation is null
    or char_length(occupation) <= 120
  );

comment on column public.profiles.occupation is
  'Optional occupation / job title, max 120 characters';
